...
export function mergeReactions(
  fromRaw: { emoji: string; count: number; userIds?: string[] }[] | undefined,
  fromColumn: { emoji: string; count: number; userIds?: string[] }[] | undefined,
) {
  if (!fromColumn && !fromRaw) return undefined;
  const merged = new Map<string, { emoji: string; count: number; userIds: Set<string> }>();
  for (const list of [fromRaw, fromColumn]) {
    if (!list) continue;
    for (const item of list) {
      const existing = merged.get(item.emoji);
      if (existing) {
  ...
  return [...merged.values()].map((r) => ({
    emoji: r.emoji,
    count: r.count,
    userIds: r.userIds.size > 0 ? [...r.userIds] : undefined,
  }));
}
