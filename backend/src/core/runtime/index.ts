...
export class GoldRuntime {
  ...
  constructor(
    ...
    if (boundAccountId) {
      void store.activateAccount(boundAccountId);
    }
    ...
    this.wireServices();
    void this.hydrateConversationsFromStore();
  }
  ...
  private wireServices() {
    ...
    this.listener.init({
      ...
      handleReactionUpdate: this.handleReactionUpdate.bind(this),
    });
  ...
  private async hydrateConversationsFromStore() {
  ...
  private async isLikelyDuplicateMessage(existing: GoldConversationMessage[], message: GoldConversationMessage) {
    if (message.providerMessageId?.trim()) {
  ...
  private async persistAttachmentLocally(messageId: string, attachment: GoldAttachment) {
    ...
    const localFileUsable = await this.state.mediaStore.isStoredFileUsable(attachment.localPath);
  ...
  private async appendConversationMessage(message: GoldConversationMessage) {
    ...
    const looksDuplicated = await this.isLikelyDuplicateMessage(existing, message);
  ...
  releaseTransientSession() {
    return this.auth.releaseTransientSession();
  }
  ...
  private async handleReactionUpdate(conversationId: string, targetGlobalMsgId: string, emoji: string, count: number, userIds: string[]) {
    ...
    const prev = messages[idx].reactions ?? [];
    const emojiIdx = prev.findIndex((r) => r.emoji === emoji);
    const next = [...prev];
    if (emojiIdx >= 0) {
      next[emojiIdx] = { ...next[emojiIdx], count: Math.max(next[emojiIdx].count, count), userIds: [...new Set([...(next[emojiIdx].userIds ?? []), ...userIds])] };
    } else {
      next.push({ emoji, count, userIds });
    }
