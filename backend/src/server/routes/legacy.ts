...
export function createLegacyRouter(
  ...
  router.get('/friends', (req, res) => {
    void (async () => {
      ...
      try {
        ...
        const contactCache = await context.runtime.getContactCache();
        const friends = refresh || contactCache.length === 0
          ? await context.runtime.listFriends()
          : contactCache;
        if (!(await context.runtime.getCurrentAccount())) {
  ...
  router.get('/groups', (req, res) => {
    void (async () => {
      ...
      try {
        const refresh = req.query.refresh === '1';
        const groupCache = await context.runtime.getGroupCache();
  ...
  router.get('/conversations/:conversationId/messages', (req, res) => {
    void (async () => {
      ...
      try {
        const messages = await context.runtime.getConversationMessages(conversationId, { since, before, limit });
  ...
  router.post('/conversations/:conversationId/sync-metadata', (req, res) => {
    void (async () => {
      ...
      try {
        ...
        broadcast({ type: 'conversation_summaries', accountId: context.accountId, conversations: await context.runtime.getConversationSummaries() });
        broadcast({ type: 'session_state', accountId: context.accountId, status: await getStatusForRuntime(context.runtime) });
        res.json(result);
      } catch (error) {
  ...
  router.get('/conversations', (_req, res) => {
    void (async () => {
      ...
      if (!context) return;
      try {
        const conversations = await context.runtime.getConversationSummaries();
