...
function DashboardPage() {
  ...
  const onReactMessage = useCallback(async (message: import('./types').Message, reaction: import('./types').MessageReactionOption) => {
    ...
    if (!cliMsgId) {
      composer.setStatusMsg('Không tìm thấy ID tin nhắn để gửi reaction');
      return;
    }
  ...
  const sidebarAccounts = useMemo(() => {
    ...
    return workspace.knownAccounts;
  }, [currentAccountId, workspace.knownAccounts, status?.account?.avatar, status?.account?.displayName, status?.account?.phoneNumber]);
