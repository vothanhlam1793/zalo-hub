...
export class GoldSessionAuth {
  ...
  private resetRuntimeState(clearStoredSession: boolean) {
  ...
  private getStoredAccountProfile() {
    const boundAccountId = this.state.boundAccountId?.trim();
    const activeAccount = boundAccountId
      ? this.state.store.listAccounts().find((account) => account.accountId === boundAccountId)
      : this.state.store.getActiveAccount();
  ...
  private mergeCurrentAccountProfile(incoming?: GoldAccountInfo) {
    const stored = this.getStoredAccountProfile();
  ...
  private async loginWithCredential(credential: GoldStoredCredential) {
    ...
    this.state.listenerStarted = false;
    this.state.listenerAttached = false;
    this.mergeCurrentAccountProfile();
  ...
  async fetchAccountInfo() {
    ...
    const api = this.state.session?.api;
    let account: GoldAccountInfo = this.mergeCurrentAccountProfile();
    ...
    this.state.currentAccount = this.mergeCurrentAccountProfile(account);
    if (this.state.currentAccount.userId) {
      this.state.store.setActiveAccount({
        accountId: this.state.currentAccount.userId,
        displayName: this.state.currentAccount.displayName,
    ...
    this.state.store.updateAccountProfile(this.state.boundAccountId ?? this.state.currentAccount.userId, {
      displayName: this.state.currentAccount.displayName,
      phoneNumber: this.state.currentAccount.phoneNumber,
      avatar: this.state.currentAccount.avatar,
    });
