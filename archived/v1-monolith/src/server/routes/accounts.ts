...
export function createAccountsRouter(
  ...
  router.put('/:accountId/profile', (req, res) => {
    ...
    try {
      accountManager.getRegistryStore().updateAccountProfile(accountId, {
        displayName: displayName || undefined,
        hubAlias: req.body?.hubAlias !== undefined ? (hubAlias || undefined) : undefined,
      });
      const runtime = accountManager.getRuntime(accountId);
      if (displayName && runtime?.getCurrentAccount()) {
        runtime.getCurrentAccount()!.displayName = displayName;
      }

      res.json({
        ok: true,
        account: accountManager.getRegistryStore().listAccounts().find((account) => account.accountId === accountId),
      });
    } catch (error) {
