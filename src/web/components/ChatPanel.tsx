          ...
          <div className="shrink-0 px-5 py-3.5 border-b border-[var(--border)] flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {typingUsers.length > 0 ? (
                ...
                <div className="text-xs text-muted-foreground mt-0.5 truncate">{activeConversationId}</div>
              )}
            </div>
            <Button
              type="button"
              ...
              title="Đồng bộ tin nhắn cũ từ Zalo"
              onClick={onSyncHistory}
              className="text-xs text-muted-foreground hover:text-[#7fa8ff] shrink-0 h-7"
            >
              {syncingHistory ? '⏳ Đồng bộ...' : '🔄 Đồng bộ'}
            </Button>
          </div>

          {(statusMsg || loadError) && (
            <div className={`shrink-0 px-4 py-2.5 text-[13px] border-b border-white/6 ${loadError ? 'bg-[rgba(255,80,80,0.1)] text-[#ff9a9a]' : 'bg-[rgba(79,122,255,0.08)] text-[#9fc0ff]'}`}>
              {loadError || statusMsg}
            </div>
          )}
