...
function MobileDashboardPage({ dashboard }: { dashboard: DashboardState }) {
  ...
  const handleOpenDirectConversation = useCallback((contact: Contact) => {
    setScreen('chat');
    onOpenDirectConversation(contact);
  }, [onOpenDirectConversation]);

  const handleOpenGroupConversation = useCallback((group: Group) => {
    setScreen('chat');
...
function DashboardPage({ mobileMode }: { mobileMode: boolean }) {
  const dashboard = useDashboardState();
  return mobileMode ? <MobileDashboardPage dashboard={dashboard} /> : <DesktopDashboardPage dashboard={dashboard} />;
}
